...
```bash
conda create -n dbx_demo_quickstart python=3.9
conda activate dbx_demo_quickstart
```
...
```
This command will create your personal repository under `/Repos/<username>/dbx_demo_quickstart`.
...
- To trigger the release pipeline, get the current version from the `dbx_demo_quickstart/__init__.py` file and tag the current code version:
```
